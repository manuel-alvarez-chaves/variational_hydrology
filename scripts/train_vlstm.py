...
def training_loop(epoch: int, period: str):
    ...
    epoch_loss_1, epoch_loss_2, epoch_total_loss, epoch_nse = [], [], [], []
    ...
    epoch_nse = sum(epoch_nse) / len(epoch_nse)
    ...
    return loss_1, loss_2, loss_total, epoch_nse, time_epoch
...
logger.info(f"{'Epoch':^5} | {'LR':^8} | {'Beta':^8} | {'Loss 1':^8} | {'Loss 2':^10} | {'Total':^8} | {'NSE':^8} | {'Time':^8} | {'Loss 1':^8} | {'Loss 2':^10} | {'Total':^8} | {'NSE':^8} | {'Time':^8} |")

time_training = time.time()
for epoch in trange(num_epochs, desc="Epochs", ncols=78, ascii=True, unit="epoch"):
    # Train
    loss_train_1, loss_train_2, loss_train_total, nse_train, time_train = training_loop(epoch, "train")
    if (epoch + 1) % num_validate_every != 0:
        logger.info(f"{epoch + 1:^5} | {lrs[epoch]:^8.1e} | {betas[epoch]:^8.1e} | {loss_train_1:^8.4f} | {loss_train_2:^10.3e} | {loss_train_total:^8.4f} | {nse_train:^8.4f} | {time_train:^8} | {'':^8} | {'':^10} | {'':^8} | {'':^8} | {'':^8} |")
        continue
    # Validate
    loss_val_1, loss_val_2, loss_val_total, nse_val, time_val = training_loop(epoch, "validate")
    logger.info(f"{epoch + 1:^5} | {lrs[epoch]:^8.1e} | {betas[epoch]:^8.1e} | {loss_train_1:^8.4f} | {loss_train_2:^10.3e} | {loss_train_total:^8.4f} | {nse_train:^8.4f} | {time_train:^8} | {loss_val_1:^8.4f} | {loss_val_2:^10.3e} | {loss_val_total:^8.4f} | {nse_val:^8.4f} | {time_val:^8} |")
